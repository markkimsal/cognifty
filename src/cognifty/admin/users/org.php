...
class Cgn_Service_Users_Org extends Cgn_Service_AdminCrud {
	...
	protected function _loadListData() {
		...
		$finder->groupBy('cgn_account.cgn_account_id');
		return $finder->find();
	}
...
//		$row[] = $d->get('ref_id');
	 ...
	 * For "organizations" we only want to edit a few rows, not all
	 * user account related fields.
	 */
	protected function _makeFormFields($f, $dataModel, $editMode=FALSE) {
		$values = $dataModel->valuesAsArray();

		$editable = array('display_name', 'org_name', 'org_id', 'ref_id', 'ref_no');
		foreach ($editable as $_e) {
			if (!isset($values[$_e])) {
				$values[$_e] = '';
			}
		}
		foreach ($values as $k=>$v) {
			if (!in_array($k, $editable)) continue;
	...
	function saveEvent($req, &$t) {
		...
		$org->set('ref_id', $req->cleanString('ref_id') );
